...
`scripts/` ディレクトリに便利なスクリプトを用意しています (Windows `.bat` / Unix `.sh` 両対応):

| スクリプト | 説明 |
|-----------|------|
| `scripts/setup` | 初期セットアップ (.env作成、依存関係インストール) |
| `scripts/docker-up` | Docker で全サービス起動 |
| `scripts/docker-down` | Docker 停止 |
| `scripts/dev` | ローカル開発モード (DB/Redisだけ Docker、アプリは直接起動) |
| `scripts/test` | Bot + API の全テスト実行 |
| `scripts/logs` | Docker ログ表示 (サービス選択) |
